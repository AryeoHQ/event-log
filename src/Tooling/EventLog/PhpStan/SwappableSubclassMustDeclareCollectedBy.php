...
final class SwappableSubclassMustDeclareCollectedBy extends Rule
{
    ...
    public function handle(Node $node, Scope $scope): void
    {
        $this->missing->each(fn (array $entry) => $this->errors->push(
            RuleErrorBuilder::message('A '.$entry['model'].' subclass must declare #['.class_basename(CollectedBy::class).'].')
                ->file($entry['file'])
                ->line($entry['line'])
                ->identifier('eventLog.Swappable.Subclass.CollectedBy.required')
                ->build()
        ));
    }
}
