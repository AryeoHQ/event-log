...
/** @extends RuleTestCase<SwappableSubclassFactoryMustExtendFactory> */
#[CoversClass(SwappableSubclassFactoryMustExtendFactory::class)]
#[CoversClass(SwappableSubclasses::class)]
final class SwappableSubclassFactoryMustExtendFactoryTest extends RuleTestCase
{
    use GetsFixtures;

    protected function getRule(): Rule
    {
        return new SwappableSubclassFactoryMustExtendFactory;
    }
    ...
    public function it_fails_when_the_named_class_is_not_ours(): void
    {
        $this->analyse([$this->getFixturePath('EventLog/SwappableSubclassWithForeignAttributes.php')], [
            ['A '.class_basename(Swappable::class).' subclass must point #['.class_basename(UseFactory::class).'] at a class extending '.class_basename(Factory::class).'.', 18],
        ]);
    }
}
