...
final class RecordableMustHaveSingleIdentifiesLoggable extends Rule
{
    /**
     * @param  Class_  $node
     */
    public function shouldHandle(Node $node, Scope $scope): bool
    {
        return ! $node->isAnonymous()
            ...
            && $this->annotatedProperties($node)->count() > 1;
    }
    ...
    {
        $this->error(
            message: class_basename(Recordable::class).' must have exactly one property annotated with #['.class_basename(IdentifiesLoggable::class).'].',
            line: $node->name?->getStartLine() ?? $node->getStartLine(),
            identifier: 'eventLog.Recordable.IdentifiesLoggable.multiple',
        );
    }
