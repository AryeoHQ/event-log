...
final class SwappableSubclassMustDeclareUseEloquentBuilder extends Rule
{
    ...
    public function handle(Node $node, Scope $scope): void
    {
        $this->missing->each(fn (array $entry) => $this->errors->push(
            RuleErrorBuilder::message('A '.$entry['model'].' subclass must declare #['.class_basename(UseEloquentBuilder::class).'].')
                ->file($entry['file'])
                ->line($entry['line'])
                ->identifier('eventLog.Swappable.Subclass.UseEloquentBuilder.required')
                ->build()
        ));
    }
}
